...
export class PasskeyService {
  ...
  async verifyRegistration(
    ...
    // If this is the first passkey for a hcaptcha user, finalize signup
    const user = await this.userService.findUserWithCache(userId);
    if (user.authProvider === 'hcaptcha' && !user.signupCompleted) {
  ...
  async verifyAuthentication(
    ...
    const verification = await verifyAuthenticationResponse({
      ...
      expectedRPID: rpID,
      authenticator: {
        credentialID: isoBase64URL.toBuffer(passkey.credentialId) as any,
